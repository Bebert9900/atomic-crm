...
export async function handleRun(
  ...
  // Orchestration depth (set by run_skill tool when self-calling)
  const depthHeader = req.headers.get("x-skill-depth");
  const skillDepth = depthHeader ? parseInt(depthHeader, 10) : 0;
  if (skillDepth > 1) {
    return Response.json(
      { error: "max_depth_exceeded", depth: skillDepth },
      { status: 400 },
    );
  }
